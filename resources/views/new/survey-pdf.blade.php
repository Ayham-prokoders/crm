@php
    $footer_contact1 = \App\Models\FooterSetting::where('name', 'footer_contact1')->first();
    $contact_message1 = $footer_contact1 ? $footer_contact1->value : '';
    $footer_contact2 = \App\Models\FooterSetting::where('name', 'footer_contact2')->first();
    $contact_message2 = $footer_contact2 ? $footer_contact2->value : '';
    $footer_contact3 = \App\Models\FooterSetting::where('name', 'footer_contact3')->first();
    $contact_message3 = $footer_contact3 ? $footer_contact3->value : '';
    $footer_contact4 = \App\Models\FooterSetting::where('name', 'footer_contact4')->first();
    $contact_message4 = $footer_contact4 ? $footer_contact4->value : '';

    // Add new footer contact settings for Paris and Singapore
    $footer_contact5 = \App\Models\FooterSetting::where('name', 'footer_contact5')->first();
    $contact_message5 = $footer_contact5 ? $footer_contact5->value : '';
    $footer_contact6 = \App\Models\FooterSetting::where('name', 'footer_contact6')->first();
    $contact_message6 = $footer_contact6 ? $footer_contact6->value : '';

    $footer_categories = \App\Models\FooterSetting::where('name', 'footer_categories')->first();
    $footer_cities = \App\Models\FooterSetting::where('name', 'footer_cities')->first();

    $footer_categories_arr = explode(',', $footer_categories ? $footer_categories->value : '');
    $footer_cities_arr = explode(',', $footer_cities ? $footer_cities->value : '');

    $_footer_categories = \Modules\Lms\Models\Category::whereIn('id', $footer_categories_arr)->get();
    $_footer_cities = \Modules\Lms\Models\City::whereIn('id', $footer_cities_arr)->get();

    $footer_whatsapp = \App\Models\FooterSetting::where('name', 'footer_whatsapp')->first();
    $_footer_whatsapp = $footer_whatsapp ? $footer_whatsapp->value : '';

    $footer_facebook = \App\Models\FooterSetting::where('name', 'footer_facebook')->first();
    $_footer_facebook = $footer_facebook ? $footer_facebook->value : '';

    $footer_twitter = \App\Models\FooterSetting::where('name', 'footer_twitter')->first();
    $_footer_twitter = $footer_twitter ? $footer_twitter->value : '';

    $footer_linkedin = \App\Models\FooterSetting::where('name', 'footer_linkedin')->first();
    $_footer_linkedin = $footer_linkedin ? $footer_linkedin->value : '';
@endphp

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>LPC System</title>

    <link rel="stylesheet" type="text/css" href="{{ asset('css/style_inst.css') }}">
    <link rel="stylesheet" type="text/css" href="{{ asset('css/star-survey.css') }}">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"
        integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">

    <link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
</head>

<style>
    @page {
        background: white !important;
        margin-top: 30px;
        size: A4;
    }

    @page:first {
        margin-top: 0 !important;
    }

    @font-face {
        font-family: 'Amiri';
        src: url('{{ asset("fonts/Amiri-Regular.ttf") }}') format('truetype');
        font-weight: normal;
        font-style: normal;
    }

    html, body {
        height: 100%;
        margin: 0;
        padding: 0;
    }

    .wrapper {
        min-height: 100%;
        display: flex;
        flex-direction: column;
    }

    .content {
        flex: 1; /* يملأ المساحة المتبقية */
    }


    body, p, span, div, table, th, td {
        direction: {{ $direction }};
        unicode-bidi: embed;
        text-align: {{ $direction === 'rtl' ? 'right' : 'left' }};
    }

    .footer {
         background-color: #0b2a4a;
        color: #ffffff;
        text-align: center;
        padding: 20px;
        font-family: Arial, sans-serif;
    }

      .footer .container {
        text-align: center;
        max-width: 1000px;
        margin: 0 auto;
        padding: 0 15px;
    }


    .logo-section {
        display: inline-block;
        vertical-align: center;
        margin-top: 50%;
        width: 100%;
    }

    .logof {
        width: 80%;
        height: auto;
    }

    .location-section {
        display: inline-block;
        vertical-align: top;
        width: 100%;
    }

    .location-table {
        width: 100%;
        border-collapse: collapse;
    }

    .location-table td {
        width: 50%;
        vertical-align: top;
        padding: 10px;
    }

    .location h4 { font-size: 16px;
        margin-bottom: 10px;
        color: #FFCC00;
    }

    .location p {
        font-size: 14px;
        margin: 5px 0;
    }

    .footer-middle {
        text-align: center;
    }

    .get-in-touch {
        margin-top: 10px;
        margin-bottom: 20px;
    }

    .footer-icon {
        display: inline-block;
        margin-right: 5px;
    }

    .footer-icon img {
        width: 20px;
        height: auto;
    }

    .footer-bottom {
        text-align: center;
        margin-top: 20px;
        font-size: 14px;
    }

    .footer-bottom ul {
        list-style: none;
        padding: 0;
        margin: 10px 0 0;
        display: inline-block;
    }

    .footer-bottom ul li {
        display: inline;
        margin: 0 10px;
    }

    .footer-bottom ul li a {
        color: #ffffff;
        text-decoration: none;
    }

    .footer-bottom ul li a:hover {
        text-decoration: underline;
    }

       .hero-section {
        height: 140px;
        background-color: #0b2a4a;
        padding: 0 20px;
        margin-bottom: 30px;

        text-align: {{ $direction === 'rtl' ? 'left' : 'right' }};
    }
    .logo2 {
        height: 80px;
        width: auto;
        vertical-align: middle;
        margin-{{ $direction === 'rtl' ? 'left' : 'right' }}: 20px;
    }
    .curv-box2 {
        display: none;
    }
</style>

<body dir="{{ $direction }}"
      style="text-align: {{ $direction === 'rtl' ? 'right' : 'left' }};
             font-family: {{ $direction === 'rtl' ? 'Amiri' : 'Arial, sans-serif' }};">
<div class="wrapper">
    <main>
        <section class="hero-section">
            <div class="navbar-container">
                <img src="{{ asset('whiteLogo.png') }}" alt="logo" class="logo2" />
                </div>
        </section>
    
    
        <section class="course-section">
            <div class="container">
                <strong style="margin-top: 15px; color: black;">{{ $form->title }}</strong>
    
                @if ($form->type == 'pre_course')
                    @php
                        $class = \Modules\Lms\Models\Classe::find($form->classe_id)->first();
                        $courseName = $class ? $class->course->name : null;
                    @endphp
                    @if ($courseName)
                        <strong style="margin-top: 5px; display: block; color: black;">Course: {{ $courseName }}</strong>
                    @endif
                @endif
    
                <p style="margin-top: 15px;margin-bottom: 10px;color: black;">{{ $form->description }}</p>
    
                <form id="surveyForm" method="POST">
                    @csrf
                    <input type="hidden" name="form_id" value="{{ $form->id }}">
    
                    <div style="margin-bottom: 20px; padding: 20px; background-color: white; border-radius: 10px;">
                        @foreach ($form->questions as $question)
                            <div class="form-group" style="page-break-inside: avoid;">
                                <label style="font-size: 1.2em; color: #D69900;">
                                    <strong>{{ $direction === 'rtl' ? 'سؤال ' : 'Question ' }} {{ $loop->iteration }}</strong>
                                </label><br>
                                <strong style="color: #012951; margin-top:15px;">{{ $question->question }}</strong>
                                @if ($question->is_required)
                                    <span class="required-star">*</span>
                                @endif
                                <br>
    
                                @switch($question->type)
                                    @case(\App\Enums\QuestionEnum::one_choice)
                                        <table style="width: 100%; border-collapse: collapse;">
                                            <tr>
                                                @foreach (json_decode($question->options, true) as $option)
                                                    <td style="padding: 5px;">
                                                        <input type="radio" name="answers[{{ $question->id }}][answer]"
                                                               value="{{ $option['title'] }}">
                                                        <label>{{ $option['title'] }}</label>
                                                    </td>
                                                @endforeach
                                            </tr>
                                        </table>
                                    @break
    
                                    @case(\App\Enums\QuestionEnum::multi_choice)
                                        <table style="width: 100%; border-collapse: collapse;">
                                            <tr>
                                                @foreach (json_decode($question->options, true) as $option)
                                                    <td style="padding: 5px;">
                                                        <input type="checkbox" name="answers[{{ $question->id }}][answer][]"
                                                               value="{{ $option['title'] }}">
                                                        <label>{{ $option['title'] }}</label>
                                                    </td>
                                                @endforeach
                                            </tr>
                                        </table>
                                    @break
    
                                    @case(\App\Enums\QuestionEnum::text)
                                        <textarea name="answers[{{ $question->id }}][answer]" rows="5"
                                                  style="width: 100%;"></textarea>
                                    @break
    
                                    @case(\App\Enums\QuestionEnum::rating)
                                        <table>
                                            <tr>
                                                @for ($i = 5; $i >= 1; $i--)
                                                    <td style="padding-right: 10px; padding-top:15px;">
                                                        <img src="{{ asset('images/star-e-svgrepo-com.svg') }}"
                                                             style="width: 16px;" />
                                                    </td>
                                                @endfor
                                            </tr>
                                        </table>
                                    @break
    
                                    @case(\App\Enums\QuestionEnum::missing_word)
                                        @php
                                            $sentence = str_replace(
                                                '__',
                                                '<input type="text" name="answers['.$question->id.'][answer]" style="width: 150px;">',
                                                $question->options,
                                            );
                                        @endphp
                                        <p>{!! $sentence !!}</p>
                                    @break
                                @endswitch
                            </div>
                        @endforeach
                    </div>
                </form>
            </div>
        </section>
    </main>
    
    <!-- <div style="page-break-after:always;"></div> -->
    {{-- footer --}}
        <footer class="footer" style="page-break-inside: avoid;">
            <div class="container">
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        {{-- Logo Section --}}
                        <td style="width: 100%; text-align: center; padding: 20px 0;">
                            <img src="{{ public_path('whiteLogo.png') }}" alt="LPC Logo" style="width: 50%; max-width: 200px; height: auto;">
                        </td>
                    </tr>
                    <tr>
                        {{-- Locations Section --}}
                        <td style="width: 100%; text-align: center; padding: 20px 0;">
                            <h3 style="color: rgb(255, 166, 0); margin-bottom: 15px;">Locations</h3>
                            <table style="width: 100%; border-collapse: collapse;">
                                <tr>
                                    <td style="width: 33.3%; vertical-align: top; padding: 10px;">
                                        <div class="location">
                                            <h4>LONDON HEAD OFFICE</h4>
                                            {!! $contact_message1 !!}
                                        </div>
                                    </td>
                                    <td style="width: 33.3%; vertical-align: top; padding: 10px;">
                                        <div class="location">
                                            <h4>DUBAI OFFICE</h4>
                                            {!! $contact_message2 !!}
                                        </div>
                                    </td>
                                    <td style="width: 33.3%; vertical-align: top; padding: 10px;">
                                        <div class="location">
                                            <h4>PARIS OFFICE</h4>
                                            {!! $contact_message5 !!}
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="width: 33.3%; vertical-align: top; padding: 10px;">
                                        <div class="location">
                                            <h4>SINGAPORE OFFICE</h4>
                                            {!! $contact_message6 !!}
                                        </div>
                                    </td>
                                    <td style="width: 33.3%; vertical-align: top; padding: 10px;">
                                        <div class="location">
                                            <h4>KUALA LUMPUR OFFICE</h4>
                                            {!! $contact_message3 !!}
                                        </div>
                                    </td>
                                    <td style="width: 33.3%; vertical-align: top; padding: 10px;">
                                        <div class="location">
                                            <h4>BARCELONA OFFICE</h4>
                                            {!! $contact_message4 !!}
                                        </div>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        {{-- Get in Touch Section --}}
                        <td style="width: 100%; text-align: center; padding: 10px 0;">
                            <div class="get-in-touch-section">
                                <h4 style="color: rgb(255, 166, 0); margin-bottom:15px;text-align: center;">Get in touch</h4>
                                <div class="get-in-touch" style="display: flex; justify-content: center; align-items: center; gap: 10px;">
                                    <a href="{{ $_footer_whatsapp }}" target="_blank">
                                        <img src="{{ public_path('new_icon_pdf/youtube.png') }}" alt="YouTube" style="width: 30px; height: auto;">
                                    </a>
                                    <a href="{{ $_footer_facebook }}" target="_blank">
    
                                        <img src="{{ public_path('new_icon_pdf/facebook.png') }}" alt="Facebook" style="width: 30px; height: auto;">
                                    </a>
                                    <a href="{{ $_footer_twitter }}" target="_blank">
                                        <img src="{{ public_path('new_icon_pdf/x.png') }}" alt="Twitter" style="width: 30px; height: auto;">
                                    </a>
                                    <a href="{{ $_footer_linkedin }}" target="_blank">
                                        <img src="{{ public_path('new_icon_pdf/linked-in.png') }}" alt="LinkedIn" style="width: 30px; height: auto;">
                                    </a>
                                </div>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        {{-- Copyrights Section --}}
                        <td style="width: 100%; text-align: center; padding: 10px 0;">
                            <hr style="border: 1px solid #ccc; margin: 10px auto;" />
                            <div class="footer-bottom">
                                <p style="margin: 0; padding-bottom: 10px; text-align: center;">Copyright © 2024 lpcentre.com All Rights Reserved.</p>
                                <ul style="list-style: none; padding: 0; margin: 0;">
                                    <li style="display: inline-block; margin: 0 5px;"><a href="{{ $website_url }}/contact" style="color: #ffffff; text-decoration: none;">Contact</a></li>
                                    <li style="display: inline-block; margin: 0 5px;"><a href="{{ $website_url }}/terms-and-conditions" style="color: #ffffff; text-decoration: none;">Terms and Conditions</a></li>
                                    <li style="display: inline-block; margin: 0 5px;"><a href="{{ $website_url }}/privacy-policy" style="color: #ffffff; text-decoration: none;">Privacy Policy</a></li>
                                    <li style="display: inline-block; margin: 0 5px;"><a href="{{ $website_url }}/quality-policy" style="color: #ffffff; text-decoration: none;">Quality Policy</a></li>
                                    <li style="display: inline-block; margin: 0 5px;"><a href="{{ $website_url }}/become-an-instructor" style="color: #ffffff; text-decoration: none;">Become an instructor</a></li>
                                </ul>
                            </div>
                        </td>
                    </tr>
                </table>
            </div>
        </footer>

</div>

</body>
</html>
